...
class Runner(AbstractEnvRunner):
    ...
    def run(self):
        ...
        x_axis_maximum=0
        y_axis_maximum=0
#내가 추가한거 Previous reward None 파트
        x_rewards_prev = None
        for _ in range(self.n_steps):
            ...
            #print(self.dones)
            '''
#내가 추가한거 1_reward 이전보다 크면

            for info in infos:
                #print(info)
                x_rewards =info.get('x')
                #print("x_rewards",x_rewards)
                #if info.get('screen_x_end')> 9000:
                    #rewards += 10
            '''
            '''
#내가 추가한거 s1_reward 이전보다 크면 + 1 받는것 -> 결과는 리워드를 얻기위해 계속 왼쪽 오른쪽으로 움직인다./
                if x_rewards_prev is not None:
                    if x_rewards > x_rewards_prev:
                        rewards+=1


            #    x_rewards+=rewards
            x_rewards_prev=x_rewards
            '''
# 2_ x axis를 구할때  maximum 보다 높으면 reward를 받는것 구현
            '''
            for info in infos:
                x_axis =info.get('x')
                print(info)
                if x_axis > x_axis_maximum:
                    rewards += 1
                    x_axis_maximum = x_axis
                #print("x_rewards",x_rewards)
                #if info.get('screen_x_end')> 9000:
                    #rewards += 10
            ...
            #print(mb_rewards)
            '''
#3_스킬을 만들어보자 앉는거랑 점프랑 같이
            #'''
            for info in infos:
                y_axis = info.get('y')
                if y_axis > y_axis_maximum:
                    rewards += 1
                    y_axis_maximum = y_axis
                x_axis =info.get('x')
                #print(info)
                if x_axis > x_axis_maximum:
                    rewards += 1
            ...
            if self.dones==True:
                ...
                x_axis = 0
            #'''
# 4_ x axis를 구할때  maximum 보다 높으면 reward를 받는것 구현 + 0으로 리셋되는거 구현
            '''
            for info in infos:
                x_axis =info.get('x')
                #print(info)
